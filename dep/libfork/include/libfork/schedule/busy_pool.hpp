...
public:
				...
				for (;;) {
					...
					this->steal_until(i, [&]() -> bool {
						return !m_root_task_in_flight.test(std::memory_order_acquire) ||
						       this->m_stop_requested.test(std::memory_order_acquire);
					});
	...
	auto schedule(basic_task<T, context, Allocator, Root> &&task) -> T {
		...
		steal_until(uid, [fut = std::addressof(fut)]() -> bool { return fut->is_ready(); });
...
private:
	...
	template <typename F> void steal_until(std::size_t uid, F &&cond) {
		//
