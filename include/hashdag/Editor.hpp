...
concept Editor = requires(const T ce) {
	...
	{
		ce.EditNode(NodeCoord<Word>{}, NodePointer<Word>{}, (typename T::NodeState *){},
...
template <std::unsigned_integral Word, StatelessEditor<Word> Editor_T> struct StatelessEditorWrapper {
	...
	inline EditType EditNode(const NodeCoord<Word> &coord, NodePointer<Word> node_ptr, auto &&, auto &&) const {
		return editor.EditNode(coord, node_ptr);
	}
