...
void DAGColorPool::create_vk_buffer() {
	m_svo_buffer = VkPagedBuffer::Create(
	    m_device_ptr,
	    ...
	    [this](const VkMemoryRequirements &mem_req) {
		    ...
		    m_config.node_bits_per_svo_page = std::max(m_config.node_bits_per_svo_page, node_bits_per_alignment);
		    return (1u << m_config.node_bits_per_svo_page) * sizeof(Node);
	    },
	    VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, {m_main_queue_ptr, m_sparse_queue_ptr});

	m_vbr_buffer = VkPagedBuffer::Create(
	    m_device_ptr,
	    std::min(m_device_ptr->GetPhysicalDevicePtr()->GetProperties().vk10.limits.sparseAddressSpaceSize,
	    ...
	    [this](const VkMemoryRequirements &mem_req) {
		    ...
		    m_config.word_bits_per_vbr_page = std::max(m_config.word_bits_per_vbr_page, word_bits_per_alignment);
		    return (1u << m_config.word_bits_per_vbr_page) * sizeof(uint32_t);
	    },
	    VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, {m_main_queue_ptr, m_sparse_queue_ptr});
}