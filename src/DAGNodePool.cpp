...
bool DAGNodePool::Flush(const myvk::SemaphoreGroup &wait_semaphores, const myvk::SemaphoreGroup &signal_semaphores,
	...
	if (!missing_gpu_pages.empty()) {
		std::vector<VmaAllocation> allocations(missing_gpu_pages.size());
		std::vector<VmaAllocationInfo> allocation_infos(missing_gpu_pages.size());
		...
		vmaAllocateMemoryPages(m_device_ptr->GetAllocatorHandle(), &m_gpu_page_memory_requirements, &create_info,
		                       missing_gpu_pages.size(), allocations.data(), allocation_infos.data());
		...
		std::vector<VkSparseMemoryBind> sparse_memory_binds(missing_gpu_pages.size());
		...
		uint32_t counter = 0;
		for (uint32_t gpu_page_id : missing_gpu_pages) {
